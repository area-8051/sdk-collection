 ...
 * Version            : V1.1
 * Date               : 2022/05/10
 * Description        : FreeRTOS routine, use hardware stack, interrupt nesting is optional, interrupt function no longer uses decoration
 *                      __attribute__((interrupt("WCH-Interrupt-fast")))，
 *                      The interrupt function is defined directly according to the ordinary function, and can only be modified with HIGHCODE.
...
{
    char  buf_str[128]; /* Need to pay attention to whether the memory space here is enough to print */
    va_list   v_args;
   ...
   (void)vsnprintf((char       *)&buf_str[0],
    ...
    /* Mutex operations, not available in interrupts */
...
{
    /* This function can be used as an example of interrupt function writing in FreeRTOS in this project */
    uint16_t flag;
    ...
    if((flag & GPIO_Pin_12) != 0)
    {
        ...
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);   /* Initiate handover requests as needed */
    }
    GPIOA_ClearITFlagBit(flag); /* clear interrupt flag */
}
